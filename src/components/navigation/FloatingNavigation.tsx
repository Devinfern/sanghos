...
export const FloatingNavigation = ({
  ...
  return <div className="hidden lg:flex items-center justify-center relative">
      ...
      opacity: 1,
      y: 0
    }} className="relative z-50" onMouseLeave={handleMouseLeave}>
        <div className="bg-white/90 backdrop-blur-lg border border-white/20 rounded-full px-6 py-3 shadow-xl shadow-black/5">
          <div className="flex items-center space-x-8">
            {navItems.map(item => <div key={item.id} className="relative" onMouseEnter={() => handleMouseEnter(item.id)}>
                {item.hasDropdown ? <button onClick={item.onClick} className={cn("flex items-center space-x-1 text-brand-slate hover:text-brand-primary transition-colors duration-200 font-medium py-2 px-3 rounded-full hover:bg-brand-primary/5", activeMenu === item.id && "text-brand-primary bg-brand-primary/5")}>
                    <span>{item.label}</span>
                    <ChevronDown className={cn("h-4 w-4 transition-transform duration-200", activeMenu === item.id && "rotate-180")} />
                  </button> : <Link to={`/${item.id}`} className="text-brand-slate hover:text-brand-primary transition-colors duration-200 font-medium py-2 px-3 rounded-full hover:bg-brand-primary/5">
                    {item.label}
                  </Link>}
              </div>)}
          </div>
      ...
      <AnimatePresence>
        {activeMenu && megaMenuData[activeMenu] && <motion.div initial={{
        opacity: 0,
        y: 5,
        scale: 0.98
      }} animate={{
        ...
        duration: 0.15,
        ease: "easeOut"
      }} className="absolute top-full left-1/2 -translate-x-1/2 mt-1 z-40 w-full max-w-6xl" onMouseEnter={() => handleMouseEnter(activeMenu)} onMouseLeave={handleMouseLeave}>
            {(() => {
          const sections = megaMenuData[activeMenu];
          const layoutConfig = getLayoutConfig(sections);
          return <div className="bg-white/95 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl shadow-black/10 p-8 mx-4">
                  {/* Active menu indicator */}
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-white/95 border-l border-t border-white/20 rotate-80 backdrop-blur-xl "></div>
                  
                  <div className={cn("gap-8", layoutConfig.useSingleColumn ? "space-y-8" : "grid grid-cols-2 gap-12")}>
                    {sections.map((section, sectionIndex) => <div key={sectionIndex} className="space-y-6">
                        <h3 className="text-sm font-semibold text-brand-slate/60 uppercase tracking-wide">
                          {section.title}
                        </h3>
                        <div className="space-y-3">
                          {section.items.map((item, itemIndex) => <Link key={itemIndex} to={item.href} className="group flex items-start space-x-4 p-4 rounded-xl hover:bg-brand-primary/5 transition-all duration-200">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center space-x-2">
                                  ...
                                  {item.badge && <span className="px-2 py-0.5 text-xs font-medium bg-brand-primary/10 text-brand-primary rounded-full">
                                      {item.badge}
                                    </span>}
                                </div>
