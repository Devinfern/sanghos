...
export default function WellnessJournal() {
  ...
  return <Card className="w-full max-w-3xl mx-auto border-brand-subtle/30 shadow-md bg-white/0">
      <CardHeader>
        <CardTitle className="text-2xl font-semibold text-brand-primary text-center">
          Wellness Journal
        </CardTitle>
        <CardDescription className="text-neutral-50 text-center">
          Share your thoughts and feelings to receive personalized retreat recommendations
        </CardDescription>
      </CardHeader>
      <CardContent className="bg-white/0">
        {!isJournalSubmitted ? <div className="space-y-4">
            {isAnalyzing ? <div className="min-h-[200px] flex items-center justify-center">
              ...
              </div> : <PlaceholdersAndVanishInput placeholders={journalPlaceholders} onChange={handleInputChange} onSubmit={handleFormSubmit} />}
            {journalEntry.length > 0 && !isAnalyzing && <Button onClick={() => analyzeJournal()} className="w-full bg-brand-primary hover:bg-brand-primary/90">
            ...
            {recommendations.length > 0 ? <div className="space-y-4">
                {recommendations.map(rec => <Card key={rec.retreatId} className="border border-brand-subtle/20 hover:border-brand-primary/30 transition-all">
                    <CardContent className="pt-4">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <Button variant="outline" onClick={() => navigateToRetreat(rec.retreatId)} className="border-brand-primary text-brand-primary hover:bg-brand-primary/5">
                          View Retreat
                        </Button>
                      </div>
                    </CardContent>
      ...
      <CardFooter className="flex justify-end border-t border-brand-subtle/10 pt-4">
        {isJournalSubmitted && <Button variant="ghost" onClick={handleReset}>
            Start Over
          </Button>}
      </CardFooter>
    </Card>;
