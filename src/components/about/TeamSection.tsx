...
const TeamSection = ({
  ...
  return <section className="py-20 bg-white">
      <div className="container mx-auto max-w-6xl px-4 md:px-6">
        ...
        <motion.div variants={staggerChildren} initial="hidden" animate={isLoaded ? "visible" : "hidden"} className="flex justify-center">
          {/* Vertical card design matching the new provided image */}
          <div className="max-w-sm w-full bg-[#F5F5F5] rounded-3xl overflow-hidden shadow-sm">
            {/* Image container taking full width */}
            <div className="w-full overflow-hidden">
              <img src={team[0].image} alt={team[0].name} className="w-full h-80 object-cover" />
            </div>
            
            {/* Content section with simple vertical layout */}
            <div className="p-8">
              {/* Role label */}
              ...
              <button className="w-full border-2 border-slate-800 text-slate-800 py-3 px-4 rounded-18px text-base font-medium hover:bg-slate-50 transition-colors">
                Connect
              </button>
            </div>
          </div>
