...
const RetreatHero = ({
  ...
  return <div className="relative min-h-[85vh] overflow-hidden bg-gradient-to-br from-sage-900 via-sage-800 to-sage-700">
      {/* Background Image with Overlay */}
      <div className="absolute inset-0">
        <OptimizedImage src="https://images.unsplash.com/photo-1469474968028-56623f02e42e?q=80&w=2070" alt="Mountain landscape with sun rays" className="w-full h-full object-cover" priority />
      ...
      <motion.div className="absolute top-20 right-16 opacity-20" variants={floatingVariants} animate="animate">
        <Leaf className="h-8 w-8 text-white" />
      </motion.div>
      
      <motion.div className="absolute top-32 left-20 opacity-15" variants={floatingVariants} animate="animate" transition={{
      delay: 2
    }}>
        <Mountain className="h-10 w-10 text-white" />
      </motion.div>

      <motion.div className="absolute bottom-32 right-32 opacity-25" variants={floatingVariants} animate="animate" transition={{
      delay: 4
    }}>
      ...
      {/* Animated Sparkles */}
      <motion.div className="absolute top-24 right-1/4" initial={{
      opacity: 0,
      scale: 0
    }} animate={isLoaded ? {
      opacity: 0.3,
      ...
      <motion.div className="absolute bottom-24 left-1/3" initial={{
      opacity: 0,
      scale: 0
    }} animate={isLoaded ? {
      opacity: 0.2,
      ...
      <div className="relative z-10 min-h-[95vh] flex items-center">
        <div className="container px-4 md:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div className="text-white" initial="hidden" animate={isLoaded ? "visible" : "hidden"} variants={containerVariants}>
              <motion.div variants={itemVariants}>
                <div className="flex items-center mb-6">
                  <motion.div className="flex items-center bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 border border-white/20" whileHover={{
                  scale: 1.05
                }} transition={{
                  type: "spring",
                  stiffness: 300
              ...
              <motion.div variants={itemVariants} className="space-y-4">
                <div className="grid grid-cols-3 gap-4 max-w-md">
                  <motion.div className="text-center p-4 bg-white/10 backdrop-blur-sm rounded-xl border border-white/20" whileHover={{
                  y: -2,
            ...
            <motion.div className="hidden lg:block relative" initial="hidden" animate={isLoaded ? "visible" : "hidden"} variants={containerVariants}>
              <motion.div variants={itemVariants} className="relative">
                {/* Journey Steps - Properly Aligned */}
                <div className="space-y-6">
                  <motion.div className="flex items-start space-x-4 p-6 bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20" whileHover={{
                  x: 10,
                  backgroundColor: "rgba(255,255,255,0.15)"
                }} transition={{
                  type: "spring",
