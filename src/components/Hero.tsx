          ...
          <div className={`transition-all duration-700 delay-500 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
            <form onSubmit={handleSearch} className="relative max-w-md mx-auto mb-10 glass-panel rounded-full overflow-hidden p-1">
              <div className="flex items-center">
                ...
                <Button type="submit" size="sm" className="absolute right-1 h-10 px-5 rounded-full">
                  Search
                </Button>
              </div>
            </form>
            ...
            <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4">
              <Button size="lg" className="group" onClick={() => navigate("/retreats")}>
                Browse Retreats
                <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Button>
              <Button size="lg" variant="outline" className="bg-white hover:bg-white/90" onClick={() => navigate("/instructors")}>
